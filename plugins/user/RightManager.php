<?php
/**
 * Contains RightManager class
 *
 * @author ahe <aheusingfeld@borderless-cms.de>
 * @package plugins
 * @version $Id$
 */
require_once 'user/Rights_DAL.php';
...
//		$this->logicObj = new RightsLogic($this);
	...
	public function printGeneralConfigForm() {
		...
		$retStr .= Factory::getObject('GuiUtility')->fillTemplate('fieldset_tpl'
					,array('id="rights"'
					,Factory::getObject('Dictionary')->getTrans('rights.addRight')
					,$form->toHtml(),null));
		return $retStr;
	}
	...
	private function createRightsTable($offset=null,$limit=null) {
		...
		return $tableObj->render(
			Factory::getObject('Dictionary')->getTrans('rights.heading'),
			'right_id',true);
	}
	...
	protected function createDeleteDialog(){
		...
		$heading = Factory::getObject('Dictionary')->getTrans('rights.h.deleteEntries');
	...
	public function checkTransactions($id=0) {
		$retVal = $this->checkForDeleteTransaction('right_id',__FILE__,__LINE__);

		// check for edit
		if(isset($_POST['editRightElement'])){
			return $this->makeCheck($this->dalObj,'editRightElement',$_POST,
				'update','right_id='.intval($_POST['right_id']));
		}

		// check for insert
		$retVal = $this->makeCheck($this->dalObj,'go_right_action');
