...
/**
 * Very old class. Contains logic for "Latest/ TOP X" plugin 
 * @todo document AND REFACTOR this
 * 
 * IMPORTANT: There are only those articles shown which are
 * located in a menu that is marked as "viewable4all==1"
...
class LatestContent {
	...
	*/
	public function latestContent()
	{
			...
			|| !(BcmsConfig::getInstance()->showTop5UserOnly) )
		{
			...
			echo '          <div id="latestContentHeader">',"\n";
	...
	*/
	...
	{
		echo '            <form id="latestForm" action="'.BcmsSystem::getParser()->getServerParameter('SCRIPT_URL')
						.'" method="post"'
	...
	function printLatestContent($latestType)
	{
		...
		if(method_exists($this, $latestType))
		{
			...
			$msg = "The \$_POST-var \"sel_latest_type\"" .
				...
				BcmsSystem::getParser()->getServerParameter('SCRIPT_URL').":"
				.BcmsSystem::getParser()->getServerParameter('REMOTE_PORT')
				."!"; // @todo Use dictionary!
		...
		for($j=0;$j<count($row);$j++)
		{
			...
			$content_id = $row[$j][0];
			$latestheader = $parser->prepareText4Preview($row[$j][1]);
	...
	*/
	function getLatestType()
	{
		...
		else
		{
			...
			$returnwert = explode(".",
				BcmsSystem::getParser()->getPostParameter('sel_latest_type'));
			return $returnwert[0];
		}
	}
