  ...
  public function __construct() {
   ...
   protected function parseFieldsIntoLayoutForm($p_sLayout,$p_aFields, $submit_button_name)
   {
	...
	$p_sLayout = '<form  id="form_contenttext" name="form_contenttext" '
	  .'action="'.BcmsFactory::getInstanceOf('Parser')->getServerParameter('SCRIPT_URI').'" method="post" enctype="'
	...
	for ($index = 0; $index < count($p_aFields); $index++) {
	  if($p_aFields[$index]['form_tag'] != 'img')
	  {
		...
		$bcmsObject = PluginManager::getPlgInstance('ObjectManager');
	...
	$element = $form->addElement('submit','article_update_preview');
	$element->setValue(Factory::getObject('Dictionary')->getTrans('Preview'));
	$p_sLayout .= $element->toHtml();
	$element = $form->addElement('submit',$submit_button_name);
	$element->setValue(Factory::getObject('Dictionary')->getTrans('continue'));
	$p_sLayout .= $element->toHtml()."\n".'</form>';
	return $p_sLayout;
   }
	...
	private function createFormElement(&$form, $p_aFields) {
		...
		if(isset($_SESSION['current_article_data']['content'][$tech_title]))
		{
   ...
   protected function filterArticleDataFromPost($p_aFields2Filter)
   {
	...
	private function phase2(&$p_refArticle, $submit_button_name) {
		...
		$lfields =
			$this->getLayoutFields($_SESSION['current_article_data']['layout_id']);
		// URGENT check where the following comes from
		...
		echo Factory::getObject('GuiUtility')->createHeading(3,'Phase2: Artikel schreiben'); // TODO use dictionary
		$guide_text = Factory::getObject('Dictionary')
			->getTrans('write_article_info2');
		echo '<div id="guide_text">'.stripcslashes($guide_text).'</div>';
		$editor_hints = Factory::getObject('Dictionary')
			->getTrans('editor_syntax_hints');
	...
	private function presetValuesFromFirstPage($lfields){
		if($lfields[0]['tech_title']=='heading'
			&& !empty($_POST['heading']))
		{
			$lfields[0]['preset_value'] =
				BcmsFactory::getInstanceOf('Parser')->getPostParameter('heading');
		}
		$notFound = true;
		$i=0;
		while($notFound && $i<count($lfields)){
				...
				&& !empty($_POST['description']))
			{
				$lfields[$i]['preset_value'] =
					BcmsFactory::getInstanceOf('Parser')->getPostParameter('description');
				$notFound=false;
			}
			$i++;
		}
	...
	private function phase1(&$p_refArticle, $submit_button_name) {
		echo PluginManager::getPlgInstance('CategoryManager')->getLogic()->createHeading(3,'Phase1: Artikel-Metainformationen pflegen');
		$guide_text = Factory::getObject('Dictionary')
			->getTrans('write_article_info1');
		echo '<div id="guide_text">'.stripcslashes($guide_text).'</div>';
		$form =& $p_refArticle->getForm('article_form',$submit_button_name
		  ,Factory::getObject('Dictionary')->getTrans('continue'));
  ...
  private function setArticlePresetValues(&$p_refArticle, $p_refHistory=null) {
	...
	if($p_refHistory == null) {
	  ...
	  return 0;
		...
		$p_refArticle->col['version']['qf_setvalue'] = (floatval($p_refHistory['version'])+1).'.0'; // URGENT die evtl. vorhandenen Versionen in der History werden hier nicht beruecksichtigt!
	  ...
	  $p_refArticle->col['language']['qf_setvalue'] = $p_refHistory['language'];
	  ...
	  $p_refArticle->col['status']['qf_setvalue'] = $p_refHistory['status'];
   ...
   public function createForm($contentObj,$articleId=null) {
		...
		if(($objectId=$_SESSION['current_article_data']['historyID'])>0) {
		...
		if(!empty($refHistoryObj)) $histDalObj->setLayoutList($refHistoryObj['fk_cat']);

		/* For security reasons the id of the current history record is not send to
		 * the client as this could be manipulated.
		 * It is read in History_DAL::checkSpecialFields()
		 */
		if(!isset($_SESSION['current_article_data']))
		{
		  $_SESSION['current_article_data'] = array('historyID' => 0
			,'contentID' => $contentId
			, 'content' => unserialize($refHistoryObj['contenttext'])
			);
		}
		$content = unserialize($refHistoryObj['contenttext']);
			...
			|| isset($_POST['article_update_image']) )
		{
			...
			if(isset($_POST['article_update_preview'])){
				echo Factory::getObject('GuiUtility')->createHeading(
			...
			$objectmanager = PluginManager::getPlgInstance('ObjectManager');
