...
class FormManager extends AbstractManager {
	...
	public function main($menuId) {
		...
		switch ($myModArray['func']) {
			case 'login':
				return BcmsSystem::getUserManager()->getLogic()->createLoginForm();
				break;
			case 'logout':
				BcmsSystem::getUserManager()->getLogic()->logout();
				break;
			case 'password':
				return BcmsSystem::getUserManager()->getLogic()->showForgotPwordDlg();
				break;
			case 'register':
	...
	private function createUserTable($offset=null,$limit=null) {
		...
		return $tableObj->render(
			BcmsSystem::getDictionaryManager()->getTrans('forms.app_heading'),
			'form_id',true);
	}
	...
	public function checkTransactions($menuId=0) {
		...
		switch ($myModArray['func']) {
			...
			case 'logout':
				return BcmsSystem::getUserManager()->getLogic()->logout();
			case 'profile':
	...
	private function createProfileEditForm($userId=0){
		...
		$parser = BcmsSystem::getParser();
		$userId = isset($_SESSION['mod']['oid']) ? $_SESSION['mod']['oid'] : BcmsSystem::getUserManager()->getUserId();
		if($parser->getGetParameter('userID')!=null)
			$userId = intval($parser->getGetParameter('userID'));

		$refGui = BcmsFactory::getInstanceOf('GuiUtility');
		$heading =
			$refGui->createHeading(3,BcmsSystem::getDictionaryManager()->getTrans('h.UserProfileEdit'));
	...
	private function showUserProfile($userId=0){
		...
		// getUserInfosByName
		$username = isset($_SESSION['mod']['oname']) ? $_SESSION['mod']['oname'] : BcmsSystem::getUserManager()->getLogic()->getUserName();
		$userID = BcmsSystem::getUserManager()->getLogic()->getUserIdByName($username);
		$refGui = BcmsFactory::getInstanceOf('GuiUtility');
		...
		if (!BcmsSystem::getUserManager()->hasRight('SHOW_ALL_USER_FIELDS')) {
