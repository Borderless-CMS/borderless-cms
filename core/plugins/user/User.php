...
/* @file Borderless CMS - the easiest and most flexible way to a valid website
...
/**
 * Logic for UserManager. This file might contain the eldest yet existing class in Bcms!!
 *
 * @since 0.2
...
class User
{
    ...
    protected function getUserIDfromDB() {
        $sessionHandler = BcmsFactory::getInstanceOf('UserSession');
    ...
    */
    ...
    protected function getUserRights($userID) {
        ...
        for ($i = 0; $i < $numrows; $i++) {
            ...
            $rightname = $result->fetchRow(DB_FETCHMODE_ORDERED,$i);
            // if successful, add rightname to rightsarray in lower case
            if(!PEAR::isError($rightname)) $rightsArray[$i] = mb_strtolower($rightname[0]);
        }
        $result->free();
    ...
    {
        $retStr = BcmsSystem::getCategoryManager()->getLogic()->createHeading(
        ...
        $retStr .= '
          <form id="loginForm" action="'
            ...
            <div id="login_username">
              <label for="username">'.BcmsSystem::getDictionaryManager()->getTrans('username')
                  .': </label>
              <input type="text" id="username" tabindex="1" name="username" accesskey="l" />
            </div>
            <div id="login_password">
              <label for="passworte">'.BcmsSystem::getDictionaryManager()->getTrans('password')
                  .': </label>
            ...
            <div id="login_return_to_prior_page">
              <label for="return_to_prior_page">'
                  .BcmsSystem::getDictionaryManager()->getTrans('return_to_prior_page')
                  .' </label>
        ...
        if($this->configInstance->showForgotPword == 1)
            $retStr .= '      <li id="password"><a href="/login/password.html">'
                .BcmsSystem::getDictionaryManager()->getTrans('forgot_password')
                .'</a></li>'."\n";
        if($this->configInstance->showRegisterUser == 1)
            $retStr .= '      <li id="register"><a href="/login/register.html">'
                  .BcmsSystem::getDictionaryManager()->getTrans('register_now')
                  .'</a></li>'."\n";
        $retStr .= '</ul>'."\n";
        return $retStr;
    }
     ...
     */
    public function storeInitialPassword($username,$pw) {
...
// ===== PROFIL-FUNKTIONEN =====
    ...
    public function forgotPassword($username,$admin=false) {
        ...
        if( empty($id) ) {
        	return false; // throw UserDataNotFoundException('username')
        }
        $email = $this->getUserEMailFromDB($username);
        if(empty($email)) {
        	return false; // throw UserDataNotFoundException('email')
        }
   ...
   '.$this->configInstance->page_title // \bug URGENT Use dictionary!!!
   .'
   ...
   $subject = 'Passworterinnerung von "'.$this->configInstance->completeSiteUrl.'"';
    ...
    public function registerUser(&$cols, $submitButtonName = 'go_user_action')
    {
        ...
        if(!array_key_exists('pword',$cols) || empty($cols['pword'])){
        	$password = $this->generatePWord($cols['username']);
        }
        $cols['pword'] = $password;
        $cols['pword_again'] = $password;
...
// ===== GET-FUNKTIONEN =====
    ...
    {
        $sql="SELECT email FROM ".$this->dbUserDataTable." WHERE (username='".$this->db->quoteSmart($username)."')";
         $result = $this->db->query($sql);
        if ($result instanceof PEAR_ERROR){
            return BcmsSystem::raiseError($result, BcmsSystem::LOGTYPE_SELECT,
...
/* @todo uncomment the salt value -> ahe: does this make sense? As this is open source, everyone can see the salt value!!
...
*/
    ...
    /**
     * FOR INTERNAL USE ONLY: Use #verifyPassword() instead
     *
...
// ===== ANMELDE- UND ABMELDE-FUNKTIONEN =====
  ...
  /* Bei einem Fehler wird als Returnvalue 1 zurueckgegeben, sonst 0 */
    function proceed_login($userID) {
          $sessionObj = BcmsFactory::getInstanceOf('UserSession');
...
//        session_unregister('content_offset');
		session_unset();
        $sessionObj->_write( session_id(), serialize($_SESSION), $this->userID);
