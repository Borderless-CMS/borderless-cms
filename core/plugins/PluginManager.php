...
// includes
include_once 'plugins/Plugin_DAL.php';
require_once 'plugins/ModEntries_DAL.php';
require_once 'plugins/ModComps_DAL.php';
require_once 'plugins/MenuModComps_DAL.php';
...
/* public methods */
    ...
    public function init($menuId) {
        ...
        $this->getPluginsCurrentCategory($menuId);
            ...
            && in_array($class,array_keys($GLOBALS['bcms_classes'])))
        {
            if($class != $this->modname) {
                $this->mainObject = BcmsFactory::getInstanceOf($class);
    ...
    /**
     * Checks whether the requested plugin is installed and in case returns
     * an instance of according manager class.
     *
     * @param String pluginName - The name of the plugin to be instantiated
     * @return AbstractManager - an object derived from AbstractManager class
     * @author ahe
    ...
    public static function getPlgInstance($pluginName)
    {
        ...
        {
            return BcmsFactory::getInstanceOf($pluginName);
        }
    }
...
/* BEGIN ********** helper methods for init process ********** */
    ...
    /**
     * Get all Plugins belonging to current category
     */
    private function getPluginsCurrentCategory($menuId) {
        if(mb_substr($_SESSION['mod']['func'],0,1)=='\''){
            // @todo is this workaround still needed? Send mail whenever it is run through!
            PluginManager::getPlgInstance('RequestLogManager')->sendBugReport();
            $_SESSION['ATTENTION_THIS_IS_A_BUG'] = '<h1>ACHTUNG, hier nachsehen '.__FILE__.' '.__LINE__.'';
            // @todo if there is no mail send within a period until 05.10.2007, this code block can be deleted!
            $_SESSION['mod']['func'] = mb_substr($_SESSION['mod']['func'],1,-1);
        }
        // get main module
        $currMainPlugin = $this->getMainPlugin($menuId);
        $currMainPlugin['func'] = (BcmsSystem::getParser()->getGetParameter('func')!=null) ? $_SESSION['mod']['func'] : $currMainPlugin['func'];
    ...
    private function getMainPlugin($menuId) {
        $typeId = BcmsSystem::getCategoryManager()->getLogic()->getType($menuId);
...
/* END ********** helper methods for init process ********** */
    ...
    public function isPluginInstalled($modname){
        ...
        if($this->dalObj->isPluginListedInDb(BcmsSystem::getParser()->prepDbStrng($modname)))
...
/**** Additional methods for PluginManager-functionality *****/
        ...
        if($this->mainObject != null) {
            ...
            for ($i = 0; $i < count($this->activePlugins); $i++) {
                ...
                if($classname!=$this->modname)
                    $currObj = BcmsFactory::getInstanceOf($classname);
                else
                    $currObj = $this;
    ...
    public function getAllPageTitles() {
        ...
        $refValue .= BcmsSystem::getCategoryManager()->getPageTitle();
        if($refValue!='')
            $refValue .= ' '.BcmsConfig::getInstance()->connector.' ';
        $refValue .= BcmsConfig::getInstance()->page_title;
        $refParser = BcmsSystem::getParser();
    ...
    public function getAllMetaDescriptions() {
        ...
        if($this->mainObject != null){
            $refValue .= $this->mainObject->getMetaDescription().' '
                            .BcmsConfig::getInstance()->connector.' ';
        }
        $refValue .= BcmsSystem::getCategoryManager()->getMetaDescription().' ';
        if($refValue!='')
            $refValue .= ' '.BcmsConfig::getInstance()->connector.' ';
        $refValue .= BcmsConfig::getInstance()->metaDescription;
        return BcmsSystem::getParser()->filterMetaDescription($refValue);
    }
    ...
    public function getAllMetaKeywords() {
        ...
        $refMenuManager = BcmsSystem::getCategoryManager();
    ...
    /**
     * Gets the meta rating for the current page
     *
     * @return String - the rating value or null
     * @author ahe
     * @since 08.03.2007 11:56:17
     */
    public function getMetaRating() {
        // @todo realize module based page rating
        $rating = BcmsConfig::getInstance()->metaRating;
        if(!empty($rating)) {
            return '    <meta name="rating" content="'.$rating.'" />';
        } else {
            return '';
        }
    }
     ...
     * @since 08.03.2007 11:56:17
     */
    public function getMetaRobotsTags() {
        // @todo realize module based page robots tags
        return BcmsConfig::getInstance()->metaRobots;
    }
    ...
    public function start($menuId) {
        $catManager = BcmsSystem::getCategoryManager();
...
/* ************   END OF PUBLIC METHODS   ************ */
    ...
    protected function createDeleteDialog(){
        ...
        $heading = BcmsSystem::getDictionaryManager()->getTrans('plg.h.deleteEntries');
    ...
    private function listModEntries($createForm=false) {
        ...
        $tableObj->setActions($this->actions); // \bug URGENT these are the actions of Plugins NOT ModEntries!!!
        $tableObj->setTranslationPrefix('plg.');
