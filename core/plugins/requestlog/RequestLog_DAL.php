...
/**
 * data abstraction layer class for request data
 *
 * @since 0.13
 * @date 16.12.2006 09:56:43
 * @author ahe <aheusingfeld@borderlessscms.de>
 * @class RequestLog_DAL
 * @ingroup requestlog
 * @package requestlog
 */
 ...
 */
	...
	public function getListForBugReport()
	{
		...
		$this->sql['list4report'] = array(
				'select' => 'requestlog.post, requestlog.get, requestlog.session_id, ' .
	...
	protected function insertRecordForCurrentRequest() {
		...
		$cols = array(
			...
			'user_id' => BcmsSystem::getUserManager()->getUserId(),
			...
			'session_id' => $parser->prepDbStrng(print_r($_SESSION,true)),
		);
	...
	{
		$where = 'user_id='.BcmsSystem::getUserManager()->getUserId()
				.' AND request_date < '.$this->db->quote(date('Y-m-d H:i:s',time()-60*BcmsConfig::getInstance()->user_request_keep_time));
	...
	public function getPluginsCatName() {
		...
		$sql = 'SELECT cat.techname '.
			...
			' ON cat.fk_type_id = modentries.me_id '. // URGENT DAL should NOT make use of other tables in this way!!!
