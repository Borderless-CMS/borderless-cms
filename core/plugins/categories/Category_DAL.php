...
class Category_DAL extends DataAbstractionLayer {
	...
	public $idx = array(
		...
		'lr_and_uo' => array(
			'type' => 'normal',
			'cols' => array('lft','rgt','status_id')
		),
		'lr_uo_status' => array(
			'type' => 'normal',
			'cols' => array('lft','rgt','user_only','status_id')
		)
	);
 ...
 */
	...
	protected function addColPresetValues() {
		...
		// assign module entries list to menu.fk_type_id
		$meObj = PluginManager::getInstance()->getModEntries_DAL();
		$dictObj = BcmsSystem::getDictionaryManager();
		$this->col['fk_type_id']['qf_vals'] = $meObj->getPlugins();
	...
	public function setLabels() {
		...
		foreach($fieldnames as $key) {
			...
			if(!in_array($key, $this->uneditableElements))
				$trans = stripslashes(BcmsSystem::getDictionaryManager()->getTrans('cat.'.$key));
	...
	public function checkSpecialFields(&$p_aCols, $func) {
		...
		$p_aCols['editor_id'] = BcmsSystem::getUserManager()->getUserId();
		$p_aCols['edit_date'] = date('YmdHis');
	}

	public function delete($menuID,$currMenuObj)
	{
		if(!BcmsSystem::getUserManager()->hasRight($currMenuObj->getDeleteRight()))
			return(false);
...
*/
	...
	public function getTreeList($techname=null,$allmenues=false) {
		...
		if($allmenues==false) $where .= ' AND (a.status_id >= '
	...
	public function getSmallTreeList($techname=null,$allmenues=false,$isLoggedIn=false) {
		$where = $this->getWhereForTreelist($techname);
		if(!$allmenues) $where .= ' AND (a.status_id >= '
			.$GLOBALS['ARTICLE_STATUS']['published'].')';// @todo use classifications for status!
		if(!$isLoggedIn)
	 ...
	 *
	 * @param int $menuId
	 * @return mixed cat_id:int or db_error:object
	 * @author ahe
	 * @date 29.01.2006 00:37:23
	 */
	public function getIdByName($techname) {
	  $mname_prepared = BcmsSystem::getParser()->filterTechName($techname);
