...
//extends GuiUtility
{
	...
	public function __construct($id, $class=null, $attributes=null) {
		...
		$this->guiObj = Factory::getObject('GuiUtility');
...
function deActivateAll (tableId, startnum,endnum) {
			...
			for ($i = 0; $i < sizeof($this->keys); $i++) {
				...
				if(mb_substr($this->keys[$i],0,1)=='-')
					$this->translatedKeys[$i] =
						Factory::getObject('Dictionary')->getTrans(mb_substr($this->keys[$i],1));
				else
					$this->translatedKeys[$i] = Factory::getObject('Dictionary')->getTrans($prefix.$this->keys[$i]);
			}
		} else {
	...
	protected function createTableFoot($keycolumn,$createForm){
		if($createForm) {
			...
			$checkboxTd = $this->guiObj->fillTemplate('th_tpl',array($attrib,$checkbox));
		...
		$action = $this->guiObj->fillTemplate('td_tpl'
			,array('class="action_select_td" colspan="'.$spanFirst.'"'
			, $action
			))."\n";
		$pageControlField = $this->getPageSwitcherWidget(true,$spanSecond);
		$tr = 	$this->guiObj->fillTemplate('tr_tpl',array(null,
			$checkboxTd.$action.$pageControlField, null));
		return '<tfoot>'."\n".$tr.'</tfoot>'."\n";
	}
	...
	protected function createTableBody($keycolumn,$createForm){
		// process table rows
		$allTRs .= $this->processTableRows($this->table['id'],
			$keycolumn,$createForm);
	...
	private function createActionSelect() {
		...
		$label = $this->guiObj->fillTemplate('label_tpl'
			...
			, Factory::getObject('Dictionary')->getTrans('choose_action')
			, null))."\n";
		...
		$submit = $this->guiObj->fillTemplate('input_tpl',
			...
			Factory::getObject('Dictionary')->getTrans('choose'),
			null))."\n";
